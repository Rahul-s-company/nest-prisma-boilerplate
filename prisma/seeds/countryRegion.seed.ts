...
async function main() {
  try {
    ...
    await prisma.$executeRaw`TRUNCATE TABLE "geo" RESTART IDENTITY CASCADE`;

    // Create new regions
    await prisma.geo.createMany({
      data: [
        { name: 'NAMER' },
        { name: 'LATAM' },
        { name: 'EMEA' },
    ...
    // Create new countries with geo_id referencing regions
    await prisma.country.createMany({
      data: [
        ...
        { name: 'Venezuela', isoCode: 'VEN' },
        { name: 'Vietnam', isoCode: 'VNM' },
    ...
    logger.log('Country region Seed data inserted successfully!');
  } catch (error) {
    logger.error('Error seeding Country region data:', error);
    process.exit(1);
  } finally {
